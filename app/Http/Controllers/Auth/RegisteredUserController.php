...
class RegisteredUserController extends Controller
{
    ...
    public function addUser(Request $request)
    {
        ...
        $request->validate([
            ...
            'expiry' => ['required_if:role,external_auditor_temp_role'],
            'phone' => [
                'required',
                'string',
                'regex:/^(\+[0-9]{1,3})?[0-9\s-]+$/',
        ...
        if($request->has('expiry')){
            $userData['expiry'] = $request->expiry;
        }
        $user = User::create($userData);
        if($user->role == 'external_auditor_temp_role'){
            UserRequest::updateOrCreate(
                ['user_id'=>$user->id],
                ['user_id'=>$user->id,'role'=>'external_auditor_temp_role','status_update_by_id'=>auth('user')->id()],
            );
        }
        $user->rpassword = $password;
        Mail::to($request->email)->send(new UserCredentials($user));
        event(new Registered($user));
        return response()->json(['status'=>true,'message' => 'Registration Successfull'], 201);
        // Auth::login($user);
    ...
    public function store(Request $request)
    {
        ...
        return response()->json(['status'=>true,'message' => 'Registration Successful ','token'=>$token], 201);
        // Auth::login($user);
    ...
    public function generateOtp(Request $request)
    {
        ...
        $where = array('email'=>$validated['email'],'phone'=>$validated['phone']);
            $validated['otp'] = $otp;
        $uOTP = UserOtp::updateOrCreate(
            $where,
            $validated 
        );
    ...
    public function validateOtp()
    {
        ...
        $uOtp  = UserOtp::where('email', request()->input('email'))
